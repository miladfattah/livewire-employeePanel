<div class="container px-6 mx-auto grid">
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
          ...
          >
            @forelse ($states as $state)
              <tr class="text-gray-700 dark:text-gray-400" >
                ...
                <td class="px-4 py-3 text-sm">
                  <x-jet-button wire:click="showEditForm({{$state->id}})" class="bg-yellow-400 hover:bg-yellow-500" >ویرایش</x-jet-button>
                  <x-jet-button  wire:click="deleteState({{$state->id}})" class="bg-red-400 hover:bg-red-500 whitespace-nowrap"> &ThinSpace; حذف &ThinSpace; </x-jet-button>
                </td>
              </tr>
            @empty
            <tr class="text-gray-700 dark:text-gray-400" >
              <small>استانی وجود ندارد</small>
            </tr>
            @endforelse
          </tbody>
        </table>
      ...
      <div class="paginate">
        {{ $states->links('vendor.livewire.simple-tailwind') }}
      </div>
    </div>
    ...
    @if ($showFormModal)
      <x-jet-modal wire:model="showFormModal" >
        ...
        <form  class="p-4" >
            ...
            <div class="mt-4">
                <x-jet-label for="name" value="{{ __('استان') }}" />
            ...
            <div class="flex items-center justify-end mt-4">
              @if(!$editModeModal)
                <x-jet-button type="button" class="ml-4" wire:click="storeState">
                    {{ __('ثبت ') }}
                </x-jet-button>
              @else
                <x-jet-button type="button" class="ml-4" wire:click="updateState">
                    {{ __('ویرایش ') }}
                </x-jet-button>
              @endif
            </div>
